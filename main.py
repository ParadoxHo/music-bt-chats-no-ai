...
class UniversalMusicBot:
    ...
    def create_tracks_keyboard(self, tracks):
        """Создает клавиатуру только с треками"""
        keyboard = []
        ...
        for i, track in enumerate(tracks):
            ...
            button_text = f"{i+1}. {title[:25]}{'...' if len(title) > 25 else ''} ({duration})"
    ...
    async def handle_find_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE, message_text: str):
        ...
        try:
            ...
            # Редактируем сообщение с результатами - ТОЛЬКО КНОПКИ и сообщение про Людмилу
            await status_msg.edit_text(
                f"<span class='tg-spoiler'>🔴 <b>ЛЮДМИЛА - ЗЛАЯ ЖЕНЩИНА</b></span>\n\n"
    ...
    async def find_multiple_tracks(self, query: str, limit: int = 3):
        ...
        async with self.search_semaphore:
            ...
            try:
                print(f"🔍 Начинаем поиск: {query} (лимит: {limit})")
    ...
    def run(self):
        ...
        print('🔴 СООБЩЕНИЕ: "ЛЮДМИЛА - ЗЛАЯ ЖЕНЩИНА" добавлено в результаты поиска')
