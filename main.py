...
class UniversalMusicBot:
    ...
    def _create_application(self):
        ...
        self.app.add_handler(CallbackQueryHandler(self.handle_button_click, pattern="^(download|search)_"))
    ...
    async def handle_button_click(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data.startswith('download_'):
            ...
            if search_type == 'more':
                await self.search_more_tracks(update, context, chat_id)
            elif search_type == 'new':
                await query.edit_message_text("🔍 Введи новый поисковый запрос после слова 'найди'")
    ...
    async def download_selected_track(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track_index: int, chat_id: int):
        ...
        await query.edit_message_text(
            ...
            f"🎵 Исполнитель: {track.get('artist', 'Неизвестно')}\n"
        ...
        try:
            ...
            await query.edit_message_text(
                f"✅ Успешно скачан: <b>{track['title']}</b>\n"
                f"🎤 {track.get('artist', 'Неизвестный исполнитель')}\n"
                f"⏱️ {self.format_duration(track.get('duration'))}\n\n"
    ...
    async def search_more_tracks(self, update: Update, context: ContextTypes.DEFAULT_TYPE, chat_id: int):
        """Выполняет поиск дополнительных треков"""
        query = update.callback_query
        
        # Получаем сохраненные данные поиска
        if chat_id not in self.active_searches:
            await query.edit_message_text("❌ Результаты поиска устарели. Начни новый поиск.")
            return
        ...
        offset = search_data.get('offset', 0) + 3
        
        await query.edit_message_text(
            f"🔍 Ищем еще треки по запросу: <code>{search_query}</code>\n"
            f"⏳ Пожалуйста, подожди...",
            parse_mode='HTML'
        )
        
        # Выполняем поиск с смещением
        tracks = await self.find_multiple_tracks(search_query, offset=offset)
        
        if not tracks:
            await query.edit_message_text(
                f"❌ Больше не найдено треков по запросу: <code>{search_query}</code>\n"
        ...
        keyboard = self.create_tracks_keyboard(tracks, search_query, offset)
        
        await query.edit_message_text(
            f"🎵 Найдено треков по запросу: <b>{search_query}</b>\n"
            f"📊 Показано: {len(tracks)} (с {offset+1}-го по {offset+len(tracks)}-й)\n\n"
            f"💡 Выбери трек для скачивания:",
            reply_markup=keyboard,
            parse_mode='HTML'
        )

    def create_tracks_keyboard(self, tracks, search_query, offset=0):
        """Создает клавиатуру с треками и дополнительными опциями"""
        keyboard = []
        ...
        for i, track in enumerate(tracks):
            ...
            button_text = f"{i+1}. {title[:30]}{'...' if len(title) > 30 else ''}"
    ...
    async def handle_find_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE, message_text: str):
        ...
        try:
            ...
            # Создаем клавиатуру с результатами
            keyboard = self.create_tracks_keyboard(tracks, query)

            # Редактируем сообщение с результатами
            await status_msg.edit_text(
                f"🎵 Найдено треков по запросу: <b>{query}</b>\n"
                f"📊 Показано: {len(tracks)} из найденных\n\n"
                f"💡 Выбери трек для скачивания:",
                reply_markup=keyboard,
                parse_mode='HTML'
            )
    ...
    async def find_multiple_tracks(self, query: str, limit: int = 3, offset: int = 0):
        """Находит несколько треков по запросу"""
        async with self.search_semaphore:
            ...
            try:
                print(f"🔍 Начинаем поиск: {query} (лимит: {limit}, смещение: {offset})")
                
                def perform_search():
                    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                        return ydl.extract_info(f"scsearch{8+offset}:{query}", download=False)
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            ...
            f"🎯 <b>Новая функция:</b> При поиске бот показывает 3 наиболее релевантных трека на кнопках!\n\n"
    ...
    def run(self):
        ...
        print('🎵 НОВАЯ ФУНКЦИЯ: Показывает 3 трека на кнопках для выбора')
        print('🔍 Улучшенный поиск: 8+ результатов + интеллектуальная фильтрация')
